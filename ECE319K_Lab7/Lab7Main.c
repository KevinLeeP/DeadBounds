...
// Runs on MSPM0G3507
// Lab 7
// Kevin Pfeffer and Charlie Nyugen
...
void FloatOutFix(float x){
  ...
  // output to ST7735 0.000cm to 2.000cm
  printf("FLOAT d=%f cm ",x); // floating point output
}
...
int main3(void){ // main3
  ...
  while(1){
    ...
    stopTime = SysTick->VAL;
    ADCtime = ((startTime-stopTime) & 0x00FFFFFF) - Offset;
    startTime = SysTick->VAL;
...
void TIMG12_IRQHandler(void){
  if((TIMG12->CPU_INT.IIDX) == 1){ // this will acknowledge
    ...
    Data = ADCin(); // sample 12-bit ADC0 channel 5, slidepot and store into mailbox
    GPIOB->DOUTTGL31_0 = GREEN;
    Time++;
    display_flag ^= 0x01; // set the semaphore
  }
}
...
int main(void){ // main5
  ...
  // wait for semaphore
    if(display_flag){
      // clear the semaphore
      display_flag = 0;
      GPIOB->DOUTTGL31_0 = RED; // toggle red LED2 on Port B, PB26 aka toggle PB26 (minimally intrusive debugging)
