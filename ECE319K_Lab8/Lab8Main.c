...
UART1_OutChar('<');
//access first three MSBs
msg = pos/1000;
pos %= 1000;
UART1_OutChar(msg + 0x30);
msg = pos/100;
pos %= 100;
UART1_OutChar(msg + 0x30);
msg = pos/10;
UART1_OutChar(msg + 0x30);
TransmitCount++; //incremenent transmitcount
GPIOB->DOUTTGL31_0 = GREEN; // toggle PB27 (minimally intrusive debugging)
}
}
...
__enable_irq();
uint32_t PositionReceived, integer, fraction;
while(1){
PositionReceived = 0;
...
PositionReceived += 100 * (UART2_InChar()- 0x30);
PositionReceived += 10 * (UART2_InChar() - 0x30);
PositionReceived += 1 * (UART2_InChar() - 0x30);
...
integer = PositionReceived / 100;
fraction = PositionReceived % 100;
printf(" %d.%.2d cm", integer, fraction);
